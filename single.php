...
<section id="pageHero">
	<div class="pageHero-inner container">
		<div class="pageHero-titleWrap">
			...
			<?php if ($post_type　 !== 'post' && $post_type !== 'attachment' && is_single()) : //カスタム投稿かどうか.
			?>
			<?php
				$post_type_object = get_post_type_object($post_type); //投稿タイプの情報を取得
				$post_type_object_label = $post_type_object->label;
				?>
			<h2 class="pageHero-title"><span><?php echo esc_html($post_type_object_label); ?></span></h2>
			<?php else : ?>
			<?php
...
<div class="contentWrap">
	<?php if ($post_type　 == 'post' && $post_type == 'attachment' && is_single()) : ?>
	<!-- ******************** contentRow ******************** -->
